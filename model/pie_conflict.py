...
class TUNED(nn.Module):
    def __init__(self, num_views, dims, num_classes, psi=0.7):
        ...
        self.mu = nn.Parameter(torch.zeros(num_classes)) # 用于高斯分布的参数
        self.sigma = nn.Parameter(torch.ones(num_classes))
    ...
    def forward(self, X, A, ce_scale=0.1, consensus_evidence_type='gaussian'):
        # 收集每个视图的证据
        evidences = dict()
        for v in range(self.num_views):
            ...
            # 为每个视图的证据添加协同证据
            # consensus_evidence = noise_scale * torch.randn_like(raw_evidence) * self.sigma + self.mu
            """
                        raw_evidence = raw_evidence.unsqueeze(0)  # 增加一个维度以适应MultiheadAttention的输入
                        consensus_evidence = consensus_evidence.unsqueeze(0)  # 增加一个维度以适应MultiheadAttention的输入
                        attn_output, _ = self.attention(consensus_evidence, raw_evidence, raw_evidence)
                        evidences[v] = F.softplus(attn_output.squeeze(0))  # 去掉增加的维度
                        """
            weights = F.softmax(self.weights[v], dim=0)
            # 加权融合
            evidences[v] = weights[0] * consensus_evidence + weights[1] * raw_evidence
